...
CONFIG_PATH_DEFAULT = Path(__file__).resolve().parents[1] / "Config" / "research.yaml"
...
def load_config(path: Path | None = None) -> Mapping[str, Any]:
    path = Path(path) if path else CONFIG_PATH_DEFAULT
    if yaml is None or not path.exists():
        return {
            "providers": {
                "openai": {"enabled": True, "model": "gpt-5.1-mini", "temperature": 0.2, "max_output_tokens": 600, "timeout_seconds": 15},
                "twitter": {"enabled": False}
            },
            "research_policy": {
                "universe": ["SOL"], "constraints": "Spot only, keine Leverage", "risk_bounds": [1,5],
