...
if [ ! -f "$FRONTEND_DIR/index.html" ] && [ ! -f "$FRONTEND_DIR/index.html" ] && [ ! -f "$FRONTEND_DIR/index.html" ]; then
...
echo "==> Waiting for Frontend (non-blocking) ..."
MAIN_HTML="index.html"
[ -f "$FRONTEND_DIR/$MAIN_HTML" ] || MAIN_HTML="index.html"
[ -f "$FRONTEND_DIR/$MAIN_HTML" ] || MAIN_HTML="index.html"
if ! wait_for_http "http://127.0.0.1:$PORT_FRONTEND/$MAIN_HTML" 10 0.5; then
  echo "WARN: Frontend did not become ready on :$PORT_FRONTEND (continuing; check $FRONTEND_LOG)"
fi
