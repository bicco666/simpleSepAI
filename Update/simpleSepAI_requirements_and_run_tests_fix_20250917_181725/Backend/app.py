...
def run_tests():
    # Dependency check: httpx is required by starlette.testclient / fastapi.testclient
    try:
        import httpx  # noqa: F401
    except Exception as e:
        return {
            "ok": False,
            "error": "Fehlende Abhängigkeit: 'httpx'. Bitte installiere zuerst die Test-Dependencies.",
            "next": "pip install -r requirements.txt  # (enthält httpx)",
            "detail": str(e),
        }
    ...
    try:
        proc = subprocess.run(cmd, cwd=ROOT_DIR, capture_output=True, text=True, timeout=180)
