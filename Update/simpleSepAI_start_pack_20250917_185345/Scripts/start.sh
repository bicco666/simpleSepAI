...
wait_for_http() {
  ...
  for ((i=1; i<=tries; i++)); do
    if curl -sS "$url" >/dev/null; then return 0; fi
    sleep "$delay"
  done
  return 1
}
...
# Prefer package import; fallback via --app-dir if needed
...
wait_for_http "http://127.0.0.1:$PORT_BACKEND/api/systemtest" 40 0.5 || { echo "Backend did not become ready"; exit 1; }
echo "==> Waiting for Frontend ..."
# Try to detect a main HTML file
...
wait_for_http "http://127.0.0.1:$PORT_FRONTEND/$MAIN_HTML" 40 0.5 || { echo "Frontend did not become ready"; exit 1; }
...
# 1) systemtest
check "GET /api/systemtest" bash -c 'curl -sS http://127.0.0.1:'"$PORT_BACKEND"'/api/systemtest | python - <<PY
import sys,json; d=json.load(sys.stdin); 
assert d.get("ok") is True and "report" in d
PY'

# 2) idea
check "GET /api/idea" bash -c 'curl -sS http://127.0.0.1:'"$PORT_BACKEND"'/api/idea | python - <<PY
import sys,json; d=json.load(sys.stdin); 
assert d.get("ok") is True and "idea" in d and "Kaufe 0,1 SOL" in d["idea"]
PY'

# 3) analysis
check "GET /api/analysis" bash -c 'curl -sS http://127.0.0.1:'"$PORT_BACKEND"'/api/analysis | python - <<PY
import sys,json; d=json.load(sys.stdin); 
assert d.get("ok") is True and "analysis" in d and "Kaufe 0,1 SOL" in d["analysis"]
PY'

# 4) execute
check "POST /api/execute" bash -c 'curl -sS -X POST http://127.0.0.1:'"$PORT_BACKEND"'/api/execute -H "Content-Type: application/json" -d "{\"sol\":0.01}" | python - <<PY
import sys,json; d=json.load(sys.stdin); 
assert d.get("ok") is True and isinstance(d.get("log"), list)
PY'

# 5) run_tests (creates date-based summary)
check "POST /api/run_tests" bash -c 'curl -sS -X POST http://127.0.0.1:'"$PORT_BACKEND"'/api/run_tests | python - <<PY
import sys,json,os; d=json.load(sys.stdin); assert d.get("ok") is True
...
if [ -n "$SUMMARY_PATH_REL" ] && [ -f "$SUMMARY_PATH_ABS" ]; then
  ...
  echo "WARN: No summary file detected"
  FAIL=$((FAIL+1))
fi
